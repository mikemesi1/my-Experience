...
int main(int argc, char **argv) {
  ...
  unsigned char *key = (unsigned char*)argv[1];	//holds the input key
  unsigned char *iv  = (unsigned char*)argv[2];	//holds the input initialization vector
...
for (int i = 0; i+1 < 16; i += 2)
{
    ...
    int hexval=0;
    //match the char with its corresponding hex value
    switch(c1){
		case 'a':
			hexval = 10 * 16;
    ...
    newkey[i/2] += char(hexval);	//set the key to its hex equivelent
}
...
for (int i = 0; i+1 < 16; i += 2)
{
    ...
    int hexval=0;
    //match the char with its corrosponding hex value
    switch(c1){
		case 'a':
			hexval = 10 * 16;
  ...
  if(1 != EVP_EncryptInit_ex(ctx, EVP_des_ede_cfb(), NULL, newkey, newiv)) handleErrors();
  ...
  //calculate the time spent encrypting
  long all=0;
  ...
  all += stop_time - start_time;	
	//cout the time
	cout << "Average time used for encrypting " << (all/1000) << " microseconds\n";
